...
const FilesPage = () => {
    ...
    const handleFileUpload = async (file) => {
        ...
        try {
            ...
            setUploadedFiles((prevFiles) => [...prevFiles, { name: file.name, lastModified: file.lastModified }]);
                    ...
                    {uploadedFiles.map((file, index) => (
                        <div key={index} className="flex items-center justify-start bg-[#F4F4F4] rounded-md h-12 w-full min-w-full max-w-full gap-3 px-5 py-9 cursor-pointer">
                            ...
                            <div className="flex flex-col w-11/12">
                                <p className="text-left text-md overflow-clip">{file.name}</p>
                                <div className="flex justify-between">
                                    <p className="text-right text-xs italic">{new Date(file.lastModified).toLocaleDateString()}</p>
