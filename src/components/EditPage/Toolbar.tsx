...
interface ToolbarProps {
    toggleActiveTool: (activeTool: string) => void; // Define the expected function type
    activeTool: string;
}

const Toolbar: React.FC<ToolbarProps> = (
    { toggleActiveTool, activeTool }) => {
    return (
        <div className="flex gap-10 bg-[#F4F4F4] rounded-3xl px-5 py-2 z-50 relative">
            ...
            <div className={`cursor-pointer w-9 h-9 flex items-center justify-center rounded-xl ${activeTool === 'eraser' ? 'bg-[#FFE7D4]' : ''
