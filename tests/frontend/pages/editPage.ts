...
class EditPage {
    ...
    async checkResetHighlightsWarningVisibility() {
        await expect(this.page.locator('div').filter({ hasText: 'WarningProceeding with this' }).nth(2)).toBeVisible();
    }
    ...
    async checkConfigBarVisibility() {
        await expect(this.page.getByText('PDFWordRearrangeIndicatorengue Prediction System fnd a Machine Learning-Based')).toBeVisible();
    }
    ...
    async checkHighlightArrangementOverlayVisibility(){
        await expect(this.page.locator('div').filter({ hasText: 'Drag to rearrange highlights' }).nth(3)).toBeVisible();
    }
