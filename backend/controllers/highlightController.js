...
const getHighlights = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .send("No highlights found for this file and user.");
    }
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch highlights" });
  }
};
...
const saveHighlights = async (req, res) => {
  ...
  try {
    ...
    if (fileQuery.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "File not found for the given filepath." });
    }
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to save highlights" });
  }
};
